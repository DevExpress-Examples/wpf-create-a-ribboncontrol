...
Namespace RibbonControl_Ex
	...
	Partial Public Class MainWindow
		...
		Private Function CreateImage(ByVal fontFamily As FontFamily) As ImageSource
			...
			Dim rtb As New RenderTargetBitmap(CInt(Math.Truncate(DimensionSize)), CInt(Math.Truncate(DimensionSize)), 96, 96, PixelFormats.Pbgra32)
			rtb.Render(v)
			Return rtb
		End Function
...
'INSTANT VB NOTE: The field items was renamed since Visual Basic does not allow fields to have the same name as other class members:
		Private Shared items_Conflict As ObservableCollection(Of FontFamily)
		Protected Shared ReadOnly Property ItemsCore() As ObservableCollection(Of FontFamily)
			Get
				If items_Conflict Is Nothing Then
					items_Conflict = New ObservableCollection(Of FontFamily)()
					For Each fam As FontFamily In Fonts.SystemFontFamilies
						If Not IsValidFamily(fam) Then
							Continue For
						End If
						items_Conflict.Add(fam)
					Next fam
				End If
